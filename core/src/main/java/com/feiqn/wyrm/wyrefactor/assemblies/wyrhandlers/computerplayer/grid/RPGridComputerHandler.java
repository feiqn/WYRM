package com.feiqn.wyrm.wyrefactor.assemblies.wyrhandlers.computerplayer.grid;
...
import static com.feiqn.wyrm.wyrefactor.helpers.interfaces.wyr.Wyr.ActorType.*;

public final class RPGridComputerHandler extends WyrComputerHandler {

    public RPGridComputerHandler(RPGridMetaHandler metaHandler) {
        super(metaHandler);
    }
    ...
    private RPGridInteraction preferredAction(RPGridUnit actor) {
    ...
    public RPGridInteraction buildAggressiveAction(RPGridUnit unit) {
        final GridPathfinder.Things currentlyAccessible = GridPathfinder.currentlyAccessibleTo((RPGridMap) h().map(), unit);
        ...
        switch(opposition.size()) {
            case 0:
                ...
                final GridPathfinder.Things potentiallyAccessible = GridPathfinder.potentiallyAccessibleTo((RPGridMap) h().map(), unit);
...
//        Gdx.app.log("ai_preferredAction", "action type: " + bestChoice.getInteractType());
    ...
    private int weightForChoice(RPGridInteraction interaction) {
        ...
        switch(interaction.getSubject().stats().getPersonality().getPersonalityType()) {
