package com.feiqn.wyrm.OLD_DATA.models.unitdata.units;
...
import static com.feiqn.wyrm.wyrefactor.assemblies.wyrhandlers.combat.math.stats.rpg.RPGStatType.SPEED;
...
public class OLD_SimpleUnit extends Image {
    ...
    protected OLD_LogicalTile occupyingTile;
    ...
    private final Array<OLD_LogicalTile> highlighted = new Array<>();

    private IronMode ironMode;

    public OLD_SimpleUnit(WYRMGame game) {
        super();
        this.game = game;
        sharedInit();
    }
    public OLD_SimpleUnit(WYRMGame game, Texture texture) {
        super(texture);
        this.game = game;
    ...
    public OLD_SimpleUnit(WYRMGame game, TextureRegion region) {
        super(region);
        this.game = game;
    ...
    private void sharedInit() {
        ...
        addListener(new ClickListener() {
            ...
            public void touchUp(InputEvent event, float x, float y, int point, int button)  {
                ...
                clicked = true;
                for(OLD_LogicalTile tile : highlighted) {
                    tile.clearHighlight();
                }
                highlighted.clear();

                final OLD_GridScreen ags = game.activeOLDGridScreen;
...
//                Gdx.app.log("unit", "touch up, " + ags.getInputMode());
                                ...
                                if(ags.conditions().tickCount() == self.modifiedSimpleSpeed()) {
                                    ...
                                    if(self.teamAlignment == TeamAlignment.PLAYER) {
                                        ...
                                        if(!isOccupyingMapObject) {
                                            ...
                                            for(OLD_SimpleUnit enemy : ags.attackableUnits) {
                                                if(enemy.teamAlignment == TeamAlignment.ENEMY || enemy.teamAlignment == TeamAlignment.STRANGER) {
                                                    enemy.addListener(new InputListener() {
                                                        ...
                                                        @Override
                                                        public void touchUp(InputEvent event, float x, float y, int point, int button) {
                                                            ags.setInputMode(OLD_GridScreen.OLD_InputMode.MENU_FOCUSED);
                                                            ags.activeUnit = null;
                                            ...
                                            switch(occupyingMapObject.OLDObjectType) {
    ...
    public void act(float delta) {
        ...
        final float timeDifference = game.activeOLDGridScreen.getClock() - previousAnimationChangeClockTime;
            ...
            case IDLE:
                try {
                    if(timeDifference >= idleAnimation.getFrameDuration()) {
                        ...
                        previousAnimationChangeClockTime = game.activeOLDGridScreen.getClock();
                    }
    ...
    protected void hoverOver() {
        ...
        game.activeOLDGridScreen.hoveredUnit = self;

        if(game.activeOLDGridScreen.activeUnit != null) return;
        if(game.activeOLDGridScreen.getInputMode() != OLD_GridScreen.OLD_InputMode.STANDARD) return;

        game.activeOLDGridScreen.getRecursionHandler().recursivelySelectReachableTiles(self);
    ...
    protected void unHover() {
        ...
        if(self.animationState == AnimationState.FLOURISH && game.activeOLDGridScreen.activeUnit != this) {
            idle();
        }

        game.activeOLDGridScreen.hoveredUnit = null;
        for(OLD_LogicalTile tile : highlighted) {
            tile.clearHighlight();
        }
        highlighted.clear();
...
//        }
    ...
    public void kill() {
        ...
        game.activeOLDGridScreen.getLogicalMap().getTileAtPositionXY(this.getColumnX(),this.getRowY()).setUnoccupied();
        game.activeOLDGridScreen.conditions().teams().removeUnitFromTeam(this);
        game.activeOLDGridScreen.conditions().removeFromTurnOrder(this);
    }
...
//    }
    ...
    public void brand(OLD_SimpleUnit brandingUnit) {
        if(!soulBranded) {
            soulBranded = true;
    ...
    public boolean brandedBy(OLD_SimpleUnit unit) { return unit == brandingUnit; }
    ...
    public static class IronMode {
        ...
        public IronMode(OLD_SimpleUnit parent) {
...
//            if(growthChanceHP < growthRates.get(RPGStatType.HEALTH)) {
//                Gdx.app.log("unit", "Ye boi defense gone up!");
//                this.maxHP++;
//                if(growthChanceHP < growthRates.get(RPGStatType.HEALTH) / 2) {
//                    Gdx.app.log("unit", "POG!");
//                    this.maxHP++;
//                    if(growthChanceHP < growthRates.get(RPGStatType.HEALTH) / 4) {
//                        Gdx.app.log("unit", "POGGERS!");
//                        this.maxHP++;
//                    }
//                }
