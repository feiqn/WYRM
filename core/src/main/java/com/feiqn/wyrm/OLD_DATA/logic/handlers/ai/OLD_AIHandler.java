...
import com.feiqn.wyrm.OLD_DATA.logic.handlers.ai.actions.OLD_AIAction;
...
//        }
    ...
    private OLD_AIAction deliberateBestOption(OLD_SimpleUnit unit) {
        ...
        final OLD_AIAction waitAction = new OLD_AIAction(game, OLD_AI_ActionType.WAIT_ACTION);
...
//                Gdx.app.log("AI Action Builder", "aggressive ai");
                ...
                if(abs.attackableUnits.size > 0) { // There are enemies I can reach this turn.
                    ...
                    if(bestAggressiveAction.getActionType() == OLD_AI_ActionType.ATTACK_ACTION) {
                    ...
                    OLD_AIAction charge = new OLD_AIAction(game, OLD_AI_ActionType.MOVE_ACTION);
                    charge.setSubjectUnit(unit);
                    charge.incrementWeight();

                    bestAggressiveAction = new OLD_AIAction(evaluateBestOrWorstCombatAction(unit, true));
                    if(bestAggressiveAction.getActionType() != OLD_AI_ActionType.WAIT_ACTION) {
...
//                Gdx.app.log("AI Action Builder", "reckless ai");
                ...
                if(abs.attackableUnits.size > 0) {
                    ...
                    if(abs.attackableUnits.size > 0) {
                        recklessAction = new OLD_AIAction(game, OLD_AI_ActionType.ATTACK_ACTION);
                        ...
                        recklessAction = new OLD_AIAction(game, OLD_AI_ActionType.MOVE_ACTION);
                        recklessAction.setSubjectUnit(unit);
                        recklessAction.setPath(shortestOLDPath);
                    }
                ...
                if(abs.attackableUnits.size > 0) {
                    for(OLD_SimpleUnit enemy : abs.attackableUnits) {
                        if(abs.getLogicalMap().distanceBetweenTiles(unit.getOccupyingTile(), enemy.getOccupyingTile()) <= unit.getSimpleReach()) {
                            final OLD_AIAction attackAction = new OLD_AIAction(game, OLD_AI_ActionType.ATTACK_ACTION);
...
//                Gdx.app.log("AI Action Builder", "patrolling ai");
                ...
                if(abs.attackableUnits.size == 0) {
                    ...
                    OLD_AIAction patrolAction = new OLD_AIAction(game, OLD_AI_ActionType.MOVE_ACTION);
...
//                Gdx.app.log("AI Action Builder", "los aggro ai");
                ...
                if(abs.attackableUnits.size > 0) {
                    for(OLD_SimpleUnit enemy : abs.attackableUnits) {
                        OLD_AIAction aggroAction = new OLD_AIAction(game, OLD_AI_ActionType.ATTACK_ACTION);
...
//                Gdx.app.log("AI Action Builder", "undefined ai");
            ...
            case ESCAPE: // Run towards escape tile
                OLD_AIAction escapeAction = new OLD_AIAction(game, OLD_AI_ActionType.ESCAPE_ACTION);
...
//        stopThinking();
//        startWaiting();
        abs.executeAction(new OLD_AIAction(game, OLD_AI_ActionType.PASS_ACTION));
    }
...
//        Gdx.app.log("AI","[aggressivePath] attackableUnits: " + abs.attackableUnits.size);
        ...
        if(bestFight.getActionType() == OLD_AI_ActionType.ATTACK_ACTION) {
...
//                Gdx.app.log("combat eval: ", "adding option");
                final OLD_AIAction option = new OLD_AIAction(game, OLD_AI_ActionType.ATTACK_ACTION);
...
//                Gdx.app.log("eval", "WORST");
            ...
            final OLD_AIAction wait = new OLD_AIAction(game, OLD_AI_ActionType.WAIT_ACTION);
